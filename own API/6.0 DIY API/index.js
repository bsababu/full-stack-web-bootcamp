...
app.get("/random", (req, res) => {
  const fetchJokes = Math.floor(Math.random()* jokes.length);
  res.json(jokes[fetchJokes]);
});

//2. GET a specific joke
app.get("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const fetchJokes = jokes.find((joke) => joke.id === id);
  res.json(fetchJokes);
});
...
//6. PATCH a joke - updating specific field
